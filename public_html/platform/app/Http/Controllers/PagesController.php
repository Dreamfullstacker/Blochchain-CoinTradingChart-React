...
class PagesController extends Controller
{
    ...
    public function clear_cache_data(Request $request)
    {
        $address = $request->input('address');
        $response = array();
        if ($address == 'all') // clear all
        {
            Cache::flush();
            $response['option'] = "clear-cache-all";
            $response['result'] = "OK";
        }
        else
        {
            $value = Cache::pull($address);
            $response['option'] = "clear-cache-item";
            $response['result'] = ($value == null) ? "NOTOK" : "OK";
        }
    ...
    public function test_bscscan_holders($address)
    {
        // $url = "https://api.bscscan.com/api?module=token&action=tokenholderlist&contractaddress=".$address."&apikey=EK9HJ8BUHGVANHPECHAIDERCDYVQJDGPE9";
        // $response = $this->send_request($url);
        // dd($response);
        ini_set('max_execution_time', 100 * 60); // 100 min
        dd($this->loop_bscscan_holders($address, 1));
    }

    private function loop_bscscan_holders($address, $page)
    {
        $url = "https://api.bscscan.com/api?module=token&action=tokenholderlist&contractaddress=".$address."&page=".$page."&offset=10000&apikey=EK9HJ8BUHGVANHPECHAIDERCDYVQJDGPE9";
        $response = $this->send_request($url);
        // $this->tx_list = array_merge($this->tx_list, $response->content->result);
        $count = count($response->content->result);
        $this->tx_count += $count;
        echo "\n";
        print_r(array($page => $this->tx_count));
        echo "\n";
        print_r($response->content->result[0]);
        if ($count == 10000) // if there is more
        {
            return $this->loop_bscscan_holders($address, $page + 1);
        }
        else // if final page
        {
            // return count($this->tx_list);
